...
## 2026-02-27 Week2 성능 측정 중 C 시나리오 100% 오류

### 1) C 시나리오 결과가 `errorRate=100%`, `avg/p95 ≈ 15000ms`
- 증상
  - `performance-week2-result.json`에서 C 시나리오가 전량 오류
  - 앱 헬스체크 타임아웃 반복
- 원인
  - 기존 스크립트가 `while(now < endTime)` 형태의 시간기반 무한 전송 구조라 고부하에서 요청이 폭주
  - worker가 가능한 한 빨리 요청을 밀어 넣어 애플리케이션의 소켓/스레드 큐가 적체됨
- 시도한 해결 방법
  - 방법 A: 기존 스크립트 유지 + 동시성만 축소
  - 방법 B: 기존 스크립트 유지 + timeout만 증가
  - 방법 C: 고정 요청 수 기반 + worker pacing + 시나리오 간 cooldown으로 스크립트 구조 변경
- 최종 해결
  - `scripts/run-week2-loadtest.ps1`를 아래 방식으로 변경
    - 시나리오별 총 요청 수 고정 (`seconds * targetRps`)
    - worker별 요청 수 분배
    - 요청 간격 pacing 적용(목표 RPS 기반)
    - 시나리오 간 cooldown(10~20초) 추가
  - 앱 8080 포트 점유 프로세스 정리 후 재기동하여 재측정
- 선택 이유
  - 방법 A를 선택하지 않은 이유: 원인(무제한 전송 구조) 자체를 제거하지 못함
  - 방법 B를 선택하지 않은 이유: 타임아웃 연장은 증상 완화일 뿐, 큐 적체를 악화시킬 수 있음
  - 방법 C를 선택한 이유: 과부하 재현을 통제하면서도 p95/TPS 비교 실험의 재현성과 신뢰도를 확보
- 재발 방지
  - 부하 스크립트 기본 원칙: 무한 전송 금지, 목표 RPS 기반 pacing 의무화
  - 측정 전 앱 헬스 체크/포트 점유 확인, 시나리오 간 cooldown 유지
