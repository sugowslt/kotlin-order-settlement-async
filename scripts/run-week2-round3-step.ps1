param(
    ...
    [string]$OutFile = "c:\backendgo\project2\performance-week2-result-round3.json"
)
    ...
    for ($worker = 1; $worker -le $Concurrency; $worker++) {
        ...
        $jobs += Start-Job -ScriptBlock {
            ...
            for ($i = 0; $i -lt $RequestsForWorker; $i++) {
                ...
                $traceId = "trace-week2-round3-{0}-{1}" -f $Worker, ([guid]::NewGuid().ToString("N").Substring(0, 8))
...
$steps = @(
    @{ name = "S1"; seconds = 90; concurrency = 80; targetRps = 500; failRatio = 0.0 },
    @{ name = "S2"; seconds = 90; concurrency = 100; targetRps = 700; failRatio = 0.0 },
    @{ name = "S3"; seconds = 90; concurrency = 120; targetRps = 900; failRatio = 0.0 }
)

$results = @()
foreach ($step in $steps) {
    Write-Host "[Step $($step.name)] sec=$($step.seconds), c=$($step.concurrency), rps=$($step.targetRps)"
    $results += Invoke-Step -StepName $step.name -Seconds $step.seconds -Concurrency $step.concurrency -TargetRps $step.targetRps -FailRatio $step.failRatio
